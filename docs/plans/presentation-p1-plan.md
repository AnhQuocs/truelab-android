# TrueLab – Kế hoạch Triển khai Presentation & UI Integration (Presentation P1 Plan)

> **Tài liệu tham chiếu chuẩn xác (Single Source of Truth cho Giai đoạn P1)**  
...
## 1. P1 Scope & README Section 7 Traceability

Đối chiếu chi tiết từng yêu cầu được cam kết trong **README Mục 7 (Application Screens)** với nền tảng Domain/Data hiện tại của TrueLab, phân loại rành mạch trạng thái triển khai trong Giai đoạn P1:
...
| **5. 🔮 Prediction Screen** | Secondary 1 | Luồng chọn trận đấu (Match Selection Flow) | **IMPLEMENT IN P1** | P1.4 | Cho phép chọn trận từ danh sách trận đấu sẵn có hoặc nhận `matchId` từ Matches Screen / Match Detail. |
| | | Trích xuất đặc trưng (Feature Extraction Context) | **IMPLEMENT IN P1** | P1.4 | Thu thập Elo (Home/Away), 5 recent matches, H2H matches, latest odds để tạo `MatchPredictionContext`. |
...
## 2. Bối cảnh & Hiện trạng Hệ thống (P1 Context & Current State)
...
> [!NOTE]
> `GetMatchDetailUseCase` là một repository-bound wrapper nằm trong nhóm legacy use cases của `MatchUseCases.kt` (được cung cấp sẵn từ `MatchDataModule` ở `:core:data`). Trong P1, Presentation sẽ gọi trực tiếp `matchRepository.getMatchDetail(matchId)` để nạp chi tiết trận đấu, giữ nguyên baseline 9 Pure Computation UseCases của D1–D3.
...
- `PredictionResult.Companion.computeWeightedScoring` và `SeasonRanking.calculateFormScore()` (tạm giữ để bảo đảm tương thích, sẽ đánh dấu `@Deprecated` trong P1.5).
- Không tự ý xóa code legacy trong các sub-phase P1.1–P1.4.

---

## 3. Kiến trúc Mục tiêu P1 (Target Architecture)
...
## 4. P1.1 — Matches Presentation Integration

Giai đoạn P1.1 được phân chia thành 2 sub-phases độc lập và rõ ràng:

### 4.1. P1.1.A — Matches List Integration (Browse, Search, Sort & Status Filter)
- **UseCases**:
...
### 4.2. P1.1.B — Match Detail Integration (MatchDetailBottomSheet & Prediction CTA)
...
## 5. P1.2 — Teams & Standings Presentation Integration
...
## 6. P1.3 — Analytics Presentation Integration (Thống kê Mô tả & Phân tích Odds)
...
## 7. P1.4 — Prediction Presentation Integration (Dự đoán Trận đấu 3 Chiều)
...
## 12. P1.5 — Kế hoạch Dọn dẹp Legacy & Xác thực Cuối (Legacy Cleanup & Verification)
...
## 13. Chiến lược Kiểm thử P1 (Testing Strategy)
...
```text
P1.1: Matches Integration
  ├── P1.1.A: Matches List (DomainUseCaseModule -> MatchUiMapper -> MatchesViewModel -> MatchesScreen -> ViewModelTest)
  └── P1.1.B: Match Detail (MatchDetailBottomSheet -> Connect click -> CTA Predict)
       │
       ▼
P1.2: Teams & Standings Integration
...
P1.3: Analytics Integration
  (AnalyticsViewModel -> AnalyticsScreen -> DescriptiveStatsCard / MultiProviderOddsCard / OddsTrendCard -> ViewModelTest)
       │
       ▼
P1.4: Prediction Integration
  (Expose MatchRepository recent/h2h -> MatchRepositoryImpl -> PredictionViewModel -> PredictionScreen -> ViewModelTest)
       │
       ▼
P1.5: Legacy Deprecation & Final Verification
  (@Deprecated legacy methods -> Full Regression: Algorithm 122 + Domain 156 + ViewModels -> P1 Final Report)
```

Mỗi phân kỳ (P1.1 $\to$ P1.5) sẽ được thực hiện độc lập, kiểm thử chặt chẽ, tự kiểm tra mã nguồn (self-review) và tạo commit atomic riêng biệt.

---

## 16. Bảng Tiêu chuẩn Hoàn thành (Definition of Done for P1)
...
- [ ] **4. Tích hợp Match Detail (P1.1.B)**: Hoàn thành `MatchDetailBottomSheet` khi nhấn vào trận đấu trên `MatchesScreen` (truy xuất qua `matchRepository.getMatchDetail`), hỗ trợ nút bấm điều hướng sang `PredictionScreen`.
